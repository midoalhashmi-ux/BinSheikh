...
| 11 | استيراد من موقع "مرآة" (مثلاً web5.topcinema.fan) يكتشف عناصر قليلة جداً أو صفر — كل روابط الأعمال بالصفحة تشير لدومين الموقع الأصلي المختلف (topcinema.io)، وفحص "نفس الأصل" (`siteSameOrigin`) كان يرفضها لاختلاف الدومين كلياً | استخراج دومين `<link rel="canonical">` من الصفحة (يشير دائماً للدومين الحقيقي بمواقع ووردبريس) واعتباره نفس الأصل أيضاً؛ `siteSameOrigin` يقبل الآن قائمة دومينات بدل دومين واحد — مؤكَّد: 0 رابط مسلسل مكتشف قبل الإصلاح، 30 بعده بنفس الصفحة | `b7900fe` — ⚠️ يحتاج `npx wrangler deploy` من `cloudflare-worker/` |
| 12 | الشاشة الرئيسية تفتح دائماً على تبويب "النتائج"؛ أزرار شاشة "المحتوى" (أفلام/مسلسلات/أنمي) مثبّتة بالكود — لا يمكن إضافة زر جديد أو تغيير ترتيبها إلا بتعديل الكود ونشر إصدار جديد | التبويب الافتراضي أصبح "أفلام/مسلسلات"؛ الأزرار تُقرأ من Firestore (`settings/homeButtons`، تُدار من AHMED-dashboard) مع رجوع تلقائي للثلاثة الافتراضية إن لم يُخصَّص شيء؛ تملأ الشاشة بالتساوي مهما كان عددها | `8ecb6ec` |
| 13 | تطبيق المشغل (sports_player) أضاف شريط عنوان أعلى شاشة المشاهدة (مثل يوتيوب) — يحتاج اسم الحلقة/الفيلم فعلياً من BinSheikh، والرابط العميق الحالي (`PlayerLauncher.openChannel`) كان يرسل `channelId` فقط بلا أي اسم | `PlayerLauncher.openChannel` يقبل الآن `title` اختيارياً ويضيفه للرابط العميق (`&title=...`، مُرمَّز بـ`Uri.encodeComponent`)؛ `ChannelCard` يمرّر `channel.title` الموجود أصلاً. تغيير غير كاسر بالكامل — رابط بلا `title` (نداءات لم تُحدَّث) يبقى يعمل تماماً كالسابق | (كوميت لاحق، مستودعان) |
